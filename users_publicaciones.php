<?php
	...
	{
		$stmt_select = $DB_con->prepare('SELECT docFile FROM tbl_docs2 WHERE docID =:uid');
		$stmt_select->execute(array(':uid'=>$_GET['delete_id']));
		$imgRow=$stmt_select->fetch(PDO::FETCH_ASSOC);
		unlink("user_files/".$imgRow['docFile']);
		
		$stmt_delete = $DB_con->prepare('DELETE FROM tbl_docs2 WHERE docID =:uid');
...
<div class="container text-center">
	...
	<div class="page-header">
    	<h1 class="h2">Plataforma de Archivos SOLFIN / <a class="btn btn-default" href="addnew_publicaciones.php"> <span class="glyphicon glyphicon-plus"></span> &nbsp; Subir Archivos </a></h1> 
    </div>
...
<table class="table" align="center">
  			<thead>
    			<tr>
      				...
      				<th scope="col">Fecha del Archivo </th>
...
<?php
	...
	//$stmt = $DB_con->prepare("SELECT * FROM tbl_docs ORDER BY docDate WHERE docType = $rol DESC LIMIT $start,$limit" );
	$stmt = $DB_con->prepare("SELECT * FROM tbl_docs2 ORDER BY docDate DESC LIMIT $start,$limit" );
	$stmt->execute();
  ...
  <tbody>
    <tr>
      ...
      <td> <p class="page-header"><?php echo $row['docSeccion']; ?></p> </td>
     <td>
			<td><a class="btn btn-primary" href="user_images/<?php echo $row['docFile']; ?>" title="click for edit" target="_blank"><span class="glyphicon glyphicon-edit"></span> Visualizar </a> </td>
			<td>
...
<?php
$stmt = $DB_con->prepare("SELECT COUNT(docID) AS total FROM tbl_docs2" );
$stmt->execute();
//$row=$stmt->rowCount();
...
for ($i=1; $i<=$total_pages; $i++) { 
            ...
            echo "<a class='btn btn-primary' href='users_publicaciones.php?page=".$i."'";
