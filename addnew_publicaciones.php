<?php
	...
	if(isset($_POST['btnsave']))
	{
		...
		if(empty($doc_seccion)){
			$errMSG = "Por favor selecione la seccion.";
		}
		...
		if(!isset($errMSG))
		{
			$stmt = $DB_con->prepare('INSERT INTO tbl_docs2(docType, docFile, docDate, docSeccion) VALUES(:dtype, :dfile, :ddate, :dseccion)');
			$stmt->bindParam(':dtype',$doctype);
			$stmt->bindParam(':dfile',$docfile);
			$stmt->bindParam(':ddate',$docdate);
			$stmt->bindParam(':dseccion',$doc_seccion);
			
			if($stmt->execute())
			{
				$successMSG = "Archivo subida exitosamente";
				header("refresh:5;users_publicaciones.php"); // redirects image view page after 5 seconds.
			}
			else
			{
...
<form method="post" enctype="multipart/form-data" class="form-horizontal">
    ...
    <tr>
    	<td><label class="control-label"> Sección </label></td>
        <td>
			<select class="form-control" name="doc_seccion">
    ...
    <tr>
        ...
        <td><input class="input-group" type="file" name="user_file" accept="*" /></td>
    </tr>
