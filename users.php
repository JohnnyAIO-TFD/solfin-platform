...
<?php
    $limit = 5;
	$start = ($page-1)*$limit;
	$departamento = $_SESSION['rol'];
	if($departamento == 'Cumplimiento'){
		...
		$stmt = $DB_con->prepare("SELECT * FROM tbl_docs WHERE docType='Contabilidad' ORDER BY docDate DESC LIMIT $start,$limit ");
	}else{
	    echo "Lo sentimos, no se corresponde a ninguno de los departamentos";
	}
